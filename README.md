...
Most automation demos focus on making something work once. This focuses on making it **safe, deterministic, and operable** in always-on environments where external systems fail, retry, or deliver duplicate events.
...
## Deployment profiles (environment mappings)

This repo is intentionally **generic** so it can be evaluated across industries. For environment-specific context, see:

- `deployment-profiles/TLDR.md` — Skills library + n8n agents across HubSpot, Google Drive, Slack, Sponsy
- `deployment-profiles/MurphyDoor.md` — Internal AI sandbox across sales, engineering, manufacturing, and support

These are short, skimmable 1-pagers that map the same execution layer to a specific operating environment.
---
